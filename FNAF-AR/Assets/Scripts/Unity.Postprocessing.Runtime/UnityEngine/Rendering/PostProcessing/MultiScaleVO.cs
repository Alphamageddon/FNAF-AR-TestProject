...
namespace UnityEngine.Rendering.PostProcessing
{
	...
	internal sealed class MultiScaleVO : IAmbientOcclusionMethod
	{
		...
		public MultiScaleVO(AmbientOcclusion settings)
		{
			...
			if (!SystemInfo.IsFormatSupported(GraphicsFormatUtility.GetGraphicsFormat(m_R8Format, isSRGB: false), FormatUsage.LoadStore) && SystemInfo.IsFormatSupported(GraphicsFormatUtility.GetGraphicsFormat(RenderTextureFormat.ARGB32, isSRGB: false), FormatUsage.LoadStore))
			{
				m_R8Format = RenderTextureFormat.ARGB32;
				float4Texture = true;
			}
			if (!SystemInfo.IsFormatSupported(GraphicsFormatUtility.GetGraphicsFormat(m_R16Format, isSRGB: false), FormatUsage.LoadStore) && SystemInfo.IsFormatSupported(GraphicsFormatUtility.GetGraphicsFormat(RenderTextureFormat.RFloat, isSRGB: false), FormatUsage.LoadStore))
			{
